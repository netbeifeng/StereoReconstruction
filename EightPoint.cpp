...
// OpenCV built-in 5-point algorithm, more stable
std::pair<Rotate, Translate> EightPointExecuter::executeFivePointAlgorithm() {
	cv::Mat R, t, mask;
	cv::Mat CameraMatrix;
	cv::Mat E;
	cv::recoverPose(m_pointSet1, m_pointSet2,
		m_sample.K_img1, cv::noArray(), m_sample.K_img2, cv::noArray(), E, R, t);
	return std::make_pair(R, t);
}

// Traditonal method for 8-point
void EightPointExecuter::initPossibleRT() {
	...
	cv::Mat R1, R2, T;
	// Will get 4 possible combinations 
	// Two R => R1, R2
	// Two T => T, -T
...
// Try to get a workable RT, if all fails, choose the best one though
...
// Validating which RT combination is valid
// Reference: https://vision.in.tum.de/_media/teaching/ss2022/mvg2022/material/sol05.zip
